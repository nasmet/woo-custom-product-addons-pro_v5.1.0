                    ...
                    foreach ($row as $field) {
                        if (isset($field['clStatus']) && $field['clStatus'] == 'visible') {
                            if(in_array($field['type'],['select', 'radio-group', 'checkbox-group','image-group','color-group'])){
                                if (is_array($field['value'])) {
                                    if(!isset($response[$field['name']]) || !is_array($response[$field['name']])){ //TODO $field['name'] can be array for repeating fields
                                        $response[$field['name']] = [];
                                    }
                                    foreach ($field['value'] as $i => $value) {
                                        $response[$field['name']][] = is_string($value) ? $value : $value['value'];
                                    }
                                }
                            }else if(in_array($field['type'],['file'])){
                                if (is_array($field['value'])) {
                                    if(!isset($response[$field['name']]) || !is_array($response[$field['name']])){
                                        $response[$field['name']] = [];
                                    }
                                    foreach ($field['value'] as $i => $value) {
                                        if(is_array($value)){
...
//                                        $response[$field['name']][] = (is_string($value) ? $value : ($value['file_name'] . ' | ' . $value['url']));
                                    }
                                ...
                                if (is_array($field['value'])) {
                                    ...
                                    foreach ($field['value'] as $i => $value) {
                                        $response[$field['name']][] = is_string($value) ? $value : '';
                                    }
...
function getPriceFromHtml($product)
{
    ...
    if ($price_html && class_exists('\DOMDocument')) {
